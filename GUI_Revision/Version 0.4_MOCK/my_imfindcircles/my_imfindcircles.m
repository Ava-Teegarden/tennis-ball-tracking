function [centers, r_estimated, metric] = my_imfindcircles(varargin)
...
[accumMatrix, gradientImg] = my_chaccum(A, radiusRange, 'Method',method,'ObjectPolarity', ...
...
[centers, metric] = my_chcenters(accumMatrix, accumThresh);
...
if (nargout > 1)
    if (length(radiusRange) == 1)
        ...
        switch (method)
            case 'phasecode'
                r_estimated = my_chradiiphcode(centers, accumMatrix, radiusRange);                
            case 'twostage'
                r_estimated = my_chradii(centers, gradientImg, radiusRange);
            otherwise
...
if isempty(parser)
    ...
    parser.addParameter('EdgeThreshold',[]);
    parser.addParameter('Sensitivity',0.85,@checkSensitivity);
end

% Parse input, replacing partial name matches with the canonical form.
if (nargin > 2) % If any name-value pairs are given
  varargin(3:end) = images.internal.remapPartialParamNames({'Method', 'ObjectPolarity', ...
